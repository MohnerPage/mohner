...
export default function EditProductPage() {
    ...
    fetch('/api/auth/me')
      ...
      .then(data => {
        if (!data.authenticated) {
          ...
          // Si está autenticado, cargar datos del producto
      ...
      .then(product => {
        if (product) {
          setForm({
            ...
            galleryUrls: product.galleryUrls ? JSON.parse(product.galleryUrls) : [],
          });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alert('¡Envase guardado con éxito! Tus cambios ya están en línea.');
  ...
  return (
    <div className="max-w-4xl pb-24">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Editar Envase</h1>
        <p className="text-sm text-slate-600">Actualiza la foto y datos de este envase. Tus cambios serán en vivo.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8 bg-white p-6 md:p-8 rounded-xl shadow-sm border border-slate-200">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="bg-slate-50 border border-dashed border-slate-300 rounded-xl p-6 transition-colors hover:bg-emerald-50">
              ...
              <div className="relative aspect-auto h-48 max-w-sm mx-auto bg-white rounded-lg overflow-hidden shadow-sm border border-slate-200 group">
                ...
                {uploading && (
                  <div className="absolute inset-0 bg-white/80 flex flex-col items-center justify-center">
                    ...
                    <p className="font-medium text-sm text-emerald-800">Cargando...</p>
                  </div>
                )}
              </div>
            ) : (
              <div onClick={() => fileInputRef.current?.click()} className="flex flex-col items-center justify-center py-6 cursor-pointer">
                {uploading ? (
                  <>
                    ...
                    <p className="text-sm font-medium text-emerald-800">Preparando tu foto...</p>
                  </>
                ) : (
                  <>
                    ...
                    <p className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded mt-2 font-bold uppercase tracking-wider">Tamaño sugerido: 800 x 800 px</p>
                  </>
                )}
              </div>
            )}
                ...
                <div className="mt-4 pt-4 border-t border-slate-200">
                  ...
                  <input name="imageUrl" value={form.imageUrl} onChange={handleChange} className="w-full text-sm p-3 bg-white border border-slate-300 rounded-lg focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20" placeholder="https://..." type="url" />
                </div>
              </div>
            </div>
            ...
            <div className="pt-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <button 
                  ...
                  className="aspect-square flex flex-col items-center justify-center border-2 border-dashed border-slate-300 rounded-lg text-slate-500 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-300 transition-colors"
                >
                  {galleryUploading ? (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input name="name" value={form.name} onChange={handleChange} required className="w-full text-sm p-3 bg-white border border-slate-300 rounded-lg focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 text-slate-800" placeholder="Ej. Envase Cuadrado 500ml" type="text" />
          </div>

          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">¿De qué material es?</label>
              <select name="material" value={form.material} onChange={handleChange} className="w-full text-sm p-3 bg-white border border-slate-300 rounded-lg focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 text-slate-800">
            ...
            <div>
              ...
              <input name="category" value={form.category} onChange={handleChange} className="w-full text-sm p-3 bg-white border border-slate-300 rounded-lg focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 text-slate-800" placeholder="Ej. Licores, Farmacia..." type="text" />
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Descripción (Para qué sirve)</label>
            <textarea name="description" value={form.description} onChange={handleChange} rows={3} className="w-full text-sm p-3 bg-white border border-slate-300 rounded-lg focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 text-slate-800 resize-y" placeholder="Ej. Frasco ideal para guardar miel o mermelada gruesa..." />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <input name="capacity" value={form.capacity} onChange={handleChange} className="w-full text-sm p-3 bg-white border border-slate-300 rounded-lg focus:border-emerald-500" placeholder="Ej. 750 ml" type="text" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Precio Unitario ($)</label>
              <input name="price" value={form.price} onChange={handleChange} className="w-full text-sm p-3 bg-white border border-slate-300 rounded-lg focus:border-emerald-500" placeholder="Ej. 12.50" type="number" step="0.01" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Colores disponibles</label>
              <input name="colors" value={form.colors} onChange={handleChange} className="w-full text-sm p-3 bg-white border border-slate-300 rounded-lg focus:border-emerald-500" placeholder="Ej. Transparente, Ámbar" type="text" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Altura Total</label>
              <input name="height" value={form.height} onChange={handleChange} className="w-full text-sm p-3 bg-white border border-slate-300 rounded-lg focus:border-emerald-500" placeholder="Ej. 13 cm" type="text" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Diámetro de la base</label>
              <input name="diameter" value={form.diameter} onChange={handleChange} className="w-full text-sm p-3 bg-white border border-slate-300 rounded-lg focus:border-emerald-500" placeholder="Ej. 7.5 cm" type="text" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Rosca / Cuello</label>
              <input name="thread" value={form.thread} onChange={handleChange} className="w-full text-sm p-3 bg-white border border-slate-300 rounded-lg focus:border-emerald-500" placeholder="Ej. Rosca 28/400" type="text" />
            </div>
          </div>
        </div>
        ...
        <div className="pt-6 mt-8 border-t border-slate-200">
          <button 
            ...
            className="w-full md:w-auto flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-8 py-3 rounded-lg text-sm font-semibold shadow-sm transition-all active:scale-[0.98] disabled:bg-slate-400"
          >
            {saving ? (
